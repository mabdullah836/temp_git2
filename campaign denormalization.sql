WITH CustomerInteractions AS (
    ...
    FROM
        ...
        ) AS data (customer_id, campaign_id, interaction_type, interaction_timestamp, device_type, source)
),
Campaigns AS (
    SELECT
        campaign_id,
    ...
    FROM
        (VALUES
            ('CAMP1', 'email', '2023-02-24 04:35:15', '2024-04-15 21:35:32', 1119.19, 'high-value', 'brand awareness'),
            ('CAMP2', 'email', '2023-10-14 00:24:33', '2024-01-17 21:09:51', 1874.55, 'high-value', 'conversion'),
            ('CAMP3', 'search ads', '2023-02-09 19:19:27', '2024-04-24 15:53:40', 3021.42, 'high-value', 'brand awareness'),
            ('CAMP4', 'social', '2023-09-20 19:17:46', '2024-03-22 22:17:53', 1106.14, 'returning', 'promotion'),
            ('CAMP5', 'social', '2023-10-15 15:31:25', '2024-01-07 04:07:04', 1795.35, 'new', 'promotion')
        ) AS data (campaign_id, channel, campaign_start, campaign_end, budget, target_audience, campaign_type)
),
Conversions AS (
    SELECT
        order_id,
    ...
    FROM
        ...
        ) AS data (order_id, customer_id, product_id, campaign_id, conversion_timestamp, order_amount)
)
SELECT
    ci.customer_id,
    ci.campaign_id,
...
FROM
    ...
    LEFT JOIN Conversions cv ON ci.customer_id = cv.customer_id AND ci.campaign_id = cv.campaign_id;